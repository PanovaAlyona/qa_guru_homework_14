...
class FindSteps:
    ...
    def find_mount_by_name(self, mount: Mountain):
        ...
        search_field = self.driver.element('[aria-label="Поиск по названию вершины"]')
    ...
    def find_routes_by_peak_name(self, mount: Mountain):
        ...
        peak_element = self.driver.element(f'//div[text()="{mount.mount_peak_name}"]')
    ...
    def check_visible_peak_pointer(self, mount: Mountain):
        ...
        elbrus_marker = self.driver.element(f'img[alt="{mount.mount_peak_name}"]')
        elbrus_marker.with_(timeout=10).should(be.visible)

    @allure.step('Проверить соответствие отображаемых маршрутов для выбранного пика')
